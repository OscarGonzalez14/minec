...
function listar_ordenes_pend_lab(){
  ...
  let tipo_lente = $("#tipo_lente_ing").val();
  let categoria_lente = $("#categoria_lente_ing").val();

  document.getElementById("select-all-env").checked = false;

  if ((inicio==undefined || inicio==null || inicio=="") || (hasta==undefined || hasta==null || hasta=="") || tipo_lente == "0"
|| categoria_lente == "0" ) {
  ...
  tabla_ordenes= $('#ordenes_pendientes_lab').DataTable({      
    ...
    "ajax":{
      ...
      type : "POST",
      dataType : "json",
      data:{inicio:inicio,hasta:hasta,tipo_lente:tipo_lente,categoria_lente:categoria_lente},           
      error: function(e){
...
function get_ordenes_procesando(){
  table_proces = $('#ordenes_finalizadas_lab').DataTable({      
    ...
    "responsive": true,
    "bInfo":true,
    "iDisplayLength": 600,//Por cada 10 registros hace una paginación
    "order": [[ 0, "desc" ]],//Ordenar (columna,orden)
      "language": { 
      "sProcessing":     "Procesando...",       
      "sLengthMenu":     "Mostrar _MENU_ registros",       
